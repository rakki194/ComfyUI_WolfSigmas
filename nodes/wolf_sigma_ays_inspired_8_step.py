import torch
import comfy.k_diffusion.sampling as k_diffusion_sampling
from ..wolf_sigma_constants import AYS_CHROMA_SIGMAS_BASE


class WolfSigmaAYSInspired8Step:
    """
    Generates an 8-step schedule inspired by AYS (Align Your Steps) for SD1.5.
    Produces 9 sigma values for 8 sampling steps by picking points from a pre-defined AYS schedule.
    The selection of which AYS base sigmas to use is now configurable.
    """

    RETURN_TYPES = ("SIGMAS",)
    FUNCTION = "get_ays_inspired_8_step_sigmas"
    CATEGORY = "sampling/sigmas_wolf/8_step"

    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "index_s8": ("INT", {"default": 0, "min": 0, "max": 10}),
                "index_s7": ("INT", {"default": 1, "min": 0, "max": 10}),
                "index_s6": ("INT", {"default": 2, "min": 0, "max": 10}),
                "index_s5": ("INT", {"default": 3, "min": 0, "max": 10}),
                "index_s4": ("INT", {"default": 4, "min": 0, "max": 10}),
                "index_s3": ("INT", {"default": 6, "min": 0, "max": 10}),
                "index_s2": ("INT", {"default": 8, "min": 0, "max": 10}),
                "index_s1": (
                    "INT",
                    {"default": 10, "min": 0, "max": 10},
                ),  # Index for the 8th (lowest non-zero) sigma
                "target_sigma_max": (
                    "FLOAT",
                    {"default": 1.0, "min": 0.0001, "max": 1000.0, "step": 0.001},
                ),
            }
        }

    def get_ays_inspired_8_step_sigmas(
        self,
        index_s8,
        index_s7,
        index_s6,
        index_s5,
        index_s4,
        index_s3,
        index_s2,
        index_s1,
        target_sigma_max,  # Added parameter
    ):
        num_steps = 8
        indices = [
            index_s8,
            index_s7,
            index_s6,
            index_s5,
            index_s4,
            index_s3,
            index_s2,
            index_s1,
        ]
        min_epsilon = 1e-7
        target_sigma_max = float(target_sigma_max)  # Ensure float

        selected_base_sigmas = []
        for i in range(num_steps):
            idx = indices[i]
            if 0 <= idx < len(AYS_CHROMA_SIGMAS_BASE):
                selected_base_sigmas.append(AYS_CHROMA_SIGMAS_BASE[idx].item())
            else:
                safe_idx = max(0, min(idx, len(AYS_CHROMA_SIGMAS_BASE) - 1))
                selected_base_sigmas.append(AYS_CHROMA_SIGMAS_BASE[safe_idx].item())
                print(
                    f"Warning: AYS 8-Step index {idx} out of bounds, used {safe_idx}."
                )

        # Sort and enforce strict decrease
        s = sorted(selected_base_sigmas, reverse=True)

        # Scale the selected sigmas if a target_sigma_max is provided and valid
        if (
            len(s) > 0
            and target_sigma_max > 0
            and abs(s[0] - target_sigma_max) > 1e-6
            and s[0] > 1e-6
        ):
            current_max_selected = s[0]
            scale_factor = target_sigma_max / current_max_selected
            s = [val * scale_factor for val in s]

        final_sigmas_list = []
        if len(s) > 0:
            final_sigmas_list.append(s[0])
            for i in range(len(s) - 1):
                next_sigma = max(s[i + 1], final_sigmas_list[-1] - min_epsilon)
                if final_sigmas_list[-1] - next_sigma < min_epsilon:
                    next_sigma = final_sigmas_list[-1] - min_epsilon
                final_sigmas_list.append(next_sigma)

        for i in range(len(final_sigmas_list)):
            final_sigmas_list[i] = max(
                final_sigmas_list[i],
                min_epsilon if i < len(final_sigmas_list) - 1 else 0.0,
            )
            if i > 0 and final_sigmas_list[i] >= final_sigmas_list[i - 1]:
                final_sigmas_list[i] = final_sigmas_list[i - 1] - min_epsilon

        final_sigmas_list.append(0.0)

        if len(final_sigmas_list) > 1 and final_sigmas_list[-2] <= 0.0:
            final_sigmas_list[-2] = min_epsilon
            if (
                len(final_sigmas_list) > 2
                and final_sigmas_list[-2] >= final_sigmas_list[-3]
            ):
                final_sigmas_list[-2] = final_sigmas_list[-3] * 0.5

        sigmas = torch.tensor(final_sigmas_list, dtype=torch.float32)

        if len(sigmas) != num_steps + 1:
            print(
                f"Warning: AYS 8-Step generated {len(sigmas)} sigmas, expected {num_steps+1}. Adjusting."
            )
            if len(sigmas) > num_steps + 1:
                sigmas = sigmas[: num_steps + 1]
            else:
                sigmas = k_diffusion_sampling.get_sigmas_karras(
                    n=num_steps, sigma_min=0.002, sigma_max=1.0, rho=7, device="cpu"
                )

        return (sigmas,)
