...
class WolfSigmaChromaBiasedKarrasNStep:
    """
    Generates an N-step Karras schedule with a bias_power, using Chroma defaults.
    bias_power > 1.0 concentrates steps towards sigma_max.
    bias_power < 1.0 concentrates steps towards sigma_min.
    """

    RETURN_TYPES = ("SIGMAS",)
    FUNCTION = "get_chroma_biased_karras_n_step_sigmas"
    ...
    def get_chroma_biased_karras_n_step_sigmas(
        self, num_steps, sigma_min, sigma_max, rho, bias_power, min_epsilon_spacing
    ):
        ...
        if (
            steps_intervals > 0
        ):  # sigs_calc might be empty if steps_intervals is 0 (though guarded)
        ...
        for i in range(steps_intervals - 1):
            lower_bound_for_next = (
                current_sigma_min  # All active sigmas >= current_sigma_min
            )
