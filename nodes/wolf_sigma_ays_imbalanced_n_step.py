...
class WolfSigmaAYSImbalancedNStep:
    """
    Generates an N-step schedule for Chroma, inspired by AYS, with a bias factor.
    ...
    def get_sigmas(
        self, num_steps, sigma_max, sigma_min, bias_factor, min_epsilon_spacing
    ):
        num_active_steps = num_steps
        if num_active_steps < 1:
            # Return a single step from sigma_max to 0 if num_steps is invalid
            return (torch.tensor([float(sigma_max), 0.0], dtype=torch.float32),)
        ...
        if num_active_steps == 1:
            ...
            for i in range(num_active_steps):
                ...
                # Ensure biased time is still within [0,1] strictly for safety, though power shouldn't change that.
