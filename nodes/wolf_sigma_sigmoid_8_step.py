...
class WolfSigmaSigmoid8Step:
    """
    Generates an 8-step schedule using a sigmoid curve.
    Produces 9 sigma values, from sigma_max down to sigma_min (potentially 0), then a final 0.0.
    """

    RETURN_TYPES = ("SIGMAS",)
    FUNCTION = "get_sigmoid_8_step_sigmas"
    ...
    def get_sigmoid_8_step_sigmas(self, sigma_max, sigma_min, steepness_factor):
        steps_intervals = 8
        ...
        for i in range(num_sigmas_to_generate - 1):
            ...
            if final_sigmas[i].item() <= 0 and i < num_sigmas_to_generate - 1:
